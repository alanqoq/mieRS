...
1.0.9。每个成功进入队列的 `/miers` 查询都会实时请求 CodexRadar，计算并生成
当前接口中可识别 GPT 与 DeepSeek 模型档位的 IQ 图片，再通过 MieBot 媒体 Outbox 回复触发消息。
...
请求超时为 20 秒，并读取当前接口仍提供的 GPT 与 DeepSeek 模型档位。缺少单个组合或该档位没有有效样本时仅省略该档位；所有可识别档位均无有效样本时才报告失败。
